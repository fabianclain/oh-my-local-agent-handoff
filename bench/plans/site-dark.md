...
It must render correctly in **both** colour schemes. The light palette is in `BRAND.md`; the dark
one is below, and it is not a tint of the light one — the brand red is 3.36:1 on the dark
background and cannot be used there.

| Role | Light | Dark |
| --- | --- | --- |
| page background | `#ffffff` | `#1e1e1e` |
| section background | `#f8f9fa` | `#262626` |
| body text | `#556270` | `#c9d1d9` |
| brand / headings | `#d9232d` | `#ff6b6b` |
| text **on** a brand-coloured button | `#ffffff` | `#1e1e1e` |

The last row is the one to read twice. White on the dark brand colour is 2.78:1 and fails; the
button's text colour has to invert with the scheme, not stay put.
...
- **A dark block that overrides only the backgrounds is the defect this plan exists to catch.**
  Every selector given a colour in the light sheet needs one in the dark block. Miss one and it
  keeps its light colour against a dark background — which renders, and is unreadable.
...
- [ ] Every text and background pair meets 4.5:1, in both schemes
- [ ] The dark block gives a colour to every selector the light sheet coloured
