...
| Hand-computed worked examples in the criteria are enough to catch a plausible-wrong answer | Only if the example is not symmetric in what it tests. `semantic`'s ordering criterion used three identical lines, so every remainder tied and every order was the same order; an implementation returning largest-remainder order instead of insertion order passed it and disagreed with the spec on 2,283 of 4,000 fuzzed trials. Five clean implementations in the morning became one wrong in nine by evening |
